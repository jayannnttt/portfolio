...
function ghosttrafficScreenshotsPlugin(): Plugin {
  const desktopDir = 'C:\\Users\\sunda\\Desktop\\GHOSTRAFFIC'
  const targetDir = path.resolve(import.meta.dirname, 'public', 'screenshots', 'ghosttraffic')
  const manifestPath = path.resolve(targetDir, 'manifest.json')
  const jsonPath = path.resolve(import.meta.dirname, 'src', 'data', 'ghosttrafficScreenshots.json')
  const tsPath = path.resolve(import.meta.dirname, 'src', 'data', 'ghosttrafficScreenshots.ts')
  ...
  const syncImages = (): string[] => {
    try {
      ...
      let files: string[] = []
      if (fs.existsSync(desktopDir)) {
        const desktopFiles = fs.readdirSync(desktopDir).filter((f) => /\.(png|jpe?g|webp|svg|avif)$/i.test(f))
        for (const f of desktopFiles) {
          try {
            const cleanName = f.replace(/\s+/g, '')
            fs.copyFileSync(path.join(desktopDir, f), path.join(targetDir, cleanName))
          } catch (e) {
            console.error('Failed copying screenshot:', f, e)
          }
        }
      ...
      const urls = sorted.map((f) => `/screenshots/ghosttraffic/${f}`)
      fs.writeFileSync(manifestPath, JSON.stringify(urls, null, 2))
      fs.writeFileSync(jsonPath, JSON.stringify(urls, null, 2))

      const tsContent = `/**
 * Automatically sorted list of GHOSTRAFFIC screenshots.
 * Synced with C:\\Users\\sunda\\Desktop\\GHOSTRAFFIC
 */
export const ghosttrafficScreenshots: string[] = ${JSON.stringify(urls, null, 2)};
`
      fs.writeFileSync(tsPath, tsContent)
      return urls
    } catch (err) {
      console.warn('GHOSTRAFFIC sync warning:', err)
      return []
    }
  }

  return {
    name: 'ghosttraffic-screenshots-plugin',
    buildStart() {
      syncImages()
    },
    configureServer(server) {
      ...
      if (fs.existsSync(desktopDir)) {
        ...
        server.watcher.on('all', (_event, filePath) => {
          if (filePath.startsWith(desktopDir)) {
            ...
            server.ws.send({
              type: 'custom',
              event: 'ghosttraffic-screenshots-updated',
              data: urls,
            })
          }
        })
      ...
      server.middlewares.use('/api/ghosttraffic-screenshots', (_req, res) => {
        const urls = syncImages()
        res.setHeader('Content-Type', 'application/json')
        res.end(JSON.stringify(urls))
      })
...
export default defineConfig({
  plugins: [react(), tailwindcss(), fitflowScreenshotsPlugin(), ghosttrafficScreenshotsPlugin()],
})
